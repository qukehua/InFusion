...
class MotionTransformer(nn.Module):
    def __init__(self,
        ...
        self.cond_feats = self.input_feats if cond_feats is None else cond_feats
    ...
    def forward(self, x, timesteps, mod=None):
        ...
        """
        B, T, D = x.shape
        if D != self.input_feats:
            raise ValueError(f"Input feature mismatch: x D={D} vs expected input_feats={self.input_feats}.")
        ...
        if mod is not None:
            ...
            if mod.shape[0] != B:
                raise ValueError(f"Condition batch mismatch: mod B={mod.shape[0]} vs x B={B}.")
            if mod.shape[1] != T:
                raise ValueError(f"Condition time mismatch: mod T={mod.shape[1]} vs x T={T}.")
